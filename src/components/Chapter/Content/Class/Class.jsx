...
const mapCapitulos =(capitulos)=>(capitulos.map((cap, index)=>(
  ...
  <Dropdown.Menu show
    ...
    <Dropdown.Header className='text-light'>{`${cap.titulo_material}`}</Dropdown.Header>
    <Dropdown.Item disabled className='text-light'>{`${cap.descripcion_material}`}</Dropdown.Item>
    <Dropdown.Divider />
    
    <Dropdown.Item bg-info shadow-1-strong>
    <a href='#' className='link-primary'background-color= 'dark'>Revisar</a></Dropdown.Item>
      </Dropdown.Menu>
  </div>
)));
